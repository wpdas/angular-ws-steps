...
return constructor}
function Steps(){return{restrict:'E',controllerAs:'vm',scope:{step:'@',enabledStep:'@',colors:'@'},controller:StepsController,transclude:!0,link:function(scope,element,attrs){if(!attrs.id){throw new Error('Attribute "id" is required on element.',element)}
var stepsNavigationElement=angular.element(document.querySelector('#'+attrs.id+' .ws-steps'));stepsNavigationElement.scope().vm.setElement(stepsNavigationElement)},bindToController:!0,template:'<div class="ws-steps">'+'<nav class="ws-steps-transclude" ng-transclude>'+'</nav>'+'<div class="steps-line-disabled"></div>'+'</div>'}}
function StepsController($scope,$attrs,StepsAPI){var vm=this;var stepsNavigation;var lastValidStep=0;if(StepsAPI.registerElementId($attrs.id)){throw new Error('Duplicate usage of an element id named: "'+$attrs.id+'"')}
function updateStepsStyle(){var currentTotalSteps=getTotalSteps();stepsNavigation.querySelectorAll('.step').forEach(function(step,index){step.style.width='calc(100% / '+currentTotalSteps+')';var stepIndex=(index+1);updateStepStyle(angular.element(step),stepIndex)},this)}
function updateStepStyle(currentStep,stepIndex){var step=parseInt(vm.step);var enabledStep=parseInt(vm.enabledStep);if(stepIndex<=step){currentStep.scope().vm.isComplete=!0}else{currentStep.scope().vm.isComplete=!1}
if(stepIndex<=enabledStep){currentStep.attr('enabled','true');currentStep.scope().vm.buttonMode=!0}else{currentStep.attr('enabled','false');currentStep.scope().vm.buttonMode=!1}
if(stepIndex===step){currentStep.attr('focused','true')}else{currentStep.attr('focused','false')}
if(step>enabledStep){vm.setStep(enabledStep)}}
function getTotalSteps(){var totalSteps=document.querySelector('#'+$attrs.id+' .ws-steps nav').children.length;if(totalSteps===0){totalSteps=1};return totalSteps}
